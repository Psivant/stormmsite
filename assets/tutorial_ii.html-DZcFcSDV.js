import{_ as n,c as e,o as s,b as a}from"./app-Ct8qdDt4.js";const i={},l=a(`<h1 id="a-random-walk-simulator-in-stormm" tabindex="-1"><a class="header-anchor" href="#a-random-walk-simulator-in-stormm"><span>A Random Walk Simulator in STORMM</span></a></h1><p>One of the most significant technical challenges undertaken in the STORMM project involves making the code&#39;s results reproducible, not just in a scientific sense (we provide the code and describe the approach in enough detail that another investigator could replicate the results) but in a numeric sense. The <a href="https://standards.ieee.org/ieee/754/6210/" target="_blank" rel="noopener noreferrer">IEEE standard for floating point arithmetic</a> admits <a href="https://grouper.ieee.org/groups/msc/ANSI_IEEE-Std-754-2019/background/addendum.html" target="_blank" rel="noopener noreferrer">some variabilities</a> and also changes in the order of operations which can change the way in which accepted roundoff errors combine. Therefore, different chipsets should be expected to produce different answers, but a highly parallel programming environment such as a GPU demands a high degree of consistency in order to spot race conditions or other subtle errors in the code. STORMM development strives for the highest degrees of numerical consistency, employing techniques such as those demonstrated in this tutorial. To showcase the capabilities, we will program a simple random walk in two dimensions, with a user-specifiable number of particles and steps, with the goal of reproducing GPU results on the CPU.</p><h2 id="random-number-generation-in-stormm" tabindex="-1"><a class="header-anchor" href="#random-number-generation-in-stormm"><span>Random Number Generation in STORMM</span></a></h2><p>STORMM makes use of the XOR-shift generators, <a href="https://prng.di.unimi.it" target="_blank" rel="noopener noreferrer">Xoroshiro128+ and Xoshiro256++</a>, to create streams of predictable pseudo-random numbers based on state vectors with small memory requirements. While useless for cryptography, the presence of &quot;jump&quot; and &quot;long jump&quot; methods in each generator makes them ideal for molecular simulations and other efforts in computational science. The particular orbit of any of the generators depends on some of the internal bit shift settings, but with any given seed the sequence begins at one point in the series and can then be fast-forwarded by the square root of the orbit&#39;s total length, 2^64^ (approximately 18 billion billion) steps in the case of Xoroshiro128+ and 2^128^ steps in the case of Xoshiro256++. There is also a &quot;long jump&quot; function in each generator, which jumps forward 2^96^ steps in Xoroshiro128+ and 2^192^ steps in Xoshiro256++.</p><p>Four billion long jumps can (theoretically) be taken in Xoroshiro128+ and billions upon billions in Xoshiro256++, but according to the author the Xoroshiro128+ generator should not be used to generate thousands of streams of random numbers, as the different segments of the generator cannot all be guaranteed to hold without detectable correlations. <a href="https://www.sciencedirect.com/science/article/pii/S0377042718306265" target="_blank" rel="noopener noreferrer">(Xoroshiro128+ also fails the Big Crush test.)</a> STORMM therefore uses the Xoshiro256++ generator in its GPU applications, but for the purposes of this tutorial it&#39;s sufficient to assign one segment of the orbit to each particle and sally forth.</p><p>The generators are easy to operate from the standpoint of a CPU program: each is encapsulated in its own C++ class and ticks forward as it produces random numbers. For GPU operations, there are special, vectorized initializers and (for kernels) a collection of <code>__device__</code> functions that can be included in any CUDA unit without needing to resort to relocatable device code (<code>-rdc</code> for those familiar with compiler commands).</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">#include &quot;../../src/Random/random.h&quot;</span>
...
<span class="line">#include &quot;../../src/DataTypes/common_types.h&quot;</span>
...
<span class="line">             double fluctation_in = 1.0,</span>
...
<span class="line">};</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>The struct constructors themselves are not displayed, but can be found in the tutorial program, <strong>/stormm/home/apps/Tutorial/randomwalk.cpp</strong>. Additions to the <code>RandomWalk</code> class for emitting these abstracts can be as follows:</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">RandomWalkWriter RandomWalk::data(const HybridTargetLevel tier) {</span>
...
<span class="line">}</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Notice how a loop over particles in the C++ code is delegated across the launch grid (all threads of all blocks) in the GPU kernel. This is a standard approach. The design also has some hidden strengths: the concept of extracting the random number generator state vector and holding it in registers throughout the entire set of simulated steps avoids what would otherwise be a lot of memory traffic (although it would mitigated to a great extent, in this case, by L1 cache). For even better memory bandwidth conservation, the position of the particle itself could be taken into registers (made into a local variable) and manipulated through the requested number of steps before the final result is written back to the arrays in main GPU memory.</p><p>In order to complete the program, we will need to add a basic user interface, for which we use the standard C <code>argc</code> and <code>argv[]</code> command line input variables. An abridged version follows. For the complete code, see <strong>/stormm/home/apps/Tutorial/tutorial_ii.cpp</strong>.</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line"></span>
<span class="line"></span>
...
<span class="line">#endif  </span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Did we succeed? Does the CPU predict and track the GPU for a simulation? Here are the results for 14592 particles simulated over 10000 steps (the number of particles was chosen large enough that multiple GPU thread blocks would be sure to participate):</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">&gt;&gt; /stormm/home/apss/Tutorial/tutorial_ii.stormm.cuda -n 14592 -x 10000 -r 8</span>
<span class="line"></span>
