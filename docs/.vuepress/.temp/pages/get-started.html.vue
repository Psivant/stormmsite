...
<ol>
<li><strong>Download and Unpack</strong>: Download the repository and unpack the source in <code v-pre>/your/STORMM/source/dir/</code>.</li>
...
<li><strong>Build</strong>: Navigate to <code v-pre>${STORMM_BUILD}</code> directory and run <code v-pre>make -j</code> to build with all possible resources.</li>
...
<ul>
<li><strong><code v-pre>-DCMAKE_BUILD_TYPE</code></strong>: Set to <code v-pre>RELEASE</code> or <code v-pre>DEBUG</code>. The <code v-pre>DEBUG</code> version removes optimizations and enables debugging flags.</li>
<li><strong><code v-pre>-DSTORMM_ENABLE_CUDA</code></strong>: Set to <code v-pre>ON</code> or <code v-pre>OFF</code> (default is <code v-pre>OFF</code>). Enables CUDA support if set to <code v-pre>ON</code>.</li>
<li><strong><code v-pre>-DSTORMM_ENABLE_RDKIT</code></strong>: Set to <code v-pre>ON</code> or <code v-pre>OFF</code> (default is <code v-pre>OFF</code>). Enables RDKit support, requiring a valid RDKit installation.</li>
...
<li><strong><code v-pre>-DCMAKE_SHOW_PTXAS</code></strong>: Set to <code v-pre>ON</code> or <code v-pre>OFF</code> (default is <code v-pre>OFF</code>). Shows PTXAS output, useful for kernel performance evaluation.</li>
...
<ul>
<li><strong>C++17 Standard</strong>: The code follows C++17 for features like enum classes, member initializers, and more.</li>
<li><strong>Coding Style</strong>: Strive for a C-like feel with C++ features like templates, private/protected members, and passing by reference.</li>
<li><strong>Iterators &amp; Pointers</strong>: Limited use of iterators, ranged loops, and unique pointers for simplicity.</li>
<li><strong>Hybrid Memory Management</strong>: Dynamic memory in a hybrid CPU/GPU environment should be allocated with Hybrid objects from the STORMM library.</li>
</ul>
...
<li><strong>Arithmetic Statements</strong>: Group operations with parentheses and follow specific formatting rules.</li>
...
<li><strong>Conditional Statements</strong>: Use <code v-pre>(!logic_expr)</code> for performance-critical cases, <code v-pre>logic_expr == false</code> for readability.</li>
</ul>
...
<li><strong>File Structure</strong>: Separate header and implementation files with corresponding <code v-pre>#include</code> guards.</li>
</ul>
...
<ul>
<li><strong>Passing Objects</strong>: Pass by const reference unless modifications are needed; then pass by pointer.</li>
<li><strong>Documentation</strong>: Place function descriptions in the <code v-pre>.cpp</code> file, Doxygen comments in the <code v-pre>.h</code> file.</li>
<li><strong>Template Conventions</strong>: Use type inference carefully, and follow the conventions outlined for templates.</li>
</ul>
<p>This guide provides an overview of the installation process, coding standards, and conventions for contributing to the STORMM codebase. For detailed information, refer to the specific sections in the documentation.</p>
</div></template>
